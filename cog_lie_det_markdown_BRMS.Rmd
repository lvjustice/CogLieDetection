...
Retreival time was log transformed to manage the skew in the data and entered into a Bayesian linear mixed effects model. Fixed effects were memory type (real, fabricated) and cognitive load (load, no-load). Results are shown in Figure X.
...
*need to think about outcome var - truncated count??*
...
#Df2 %>% group_by(mem_type) %>% count(stim_serial_order)

pirateplot(stim_serial_order ~ mem_type, data = Df)


#poisson model
expt2_serial_order <- brm(stim_serial_order ~ mem_type + (mem_type|ID) + (mem_type|mem_cue), data = Df2, family = "poisson")
...
#neg binomial model
expt2_serial_order_nb <- brm(stim_serial_order ~ mem_type + (mem_type|ID) + (mem_type|mem_cue), data = Df2, family = "negbinomial", cores = 8)

#check estimates, model and convergence
expt2_serial_order_summ <- summary(expt2_serial_order_nb )
expt2_serial_order_plot <- plot(expt2_serial_order_nb )
expt2_serial_order_check <- pp_check(expt2_serial_order_nb )
expt2_serial_order_margeff <- marginal_effects(expt2_serial_order_nb )

# Plot model
plot_model(expt2_serial_order_nb)

#model summary table
tab_model(expt2_serial_order_nb, show.hdi50 = FALSE, show.re.var = FALSE, show.r2 = FALSE, show.icc = FALSE)

#model comparisons: poisson vs negbinomial
serial_order_WAIC <- waic(expt2_serial_order, expt2_serial_order_nb)
serial_order_LOO <- loo(expt2_serial_order, expt2_serial_order_nb)

```

Two distributions used for analysing count data were implemented: Poisson and negative binomial. This was done since Poisson models do not produce reliable estimates when there is over-dispersion in the data, and evidence of over-disperson can be found when the count mean and variance are similar. In our case, the values were similar, the count mean was `r round(mean(Df2$stim_serial_order), 3)` while the variance was `r round(var(Df2$stim_serial_order), 3)`. The two models were run, investigating number of digits recalled in serial order as a function of memory type. To determine which model fit the data the best, model comparisons were run using Watanabe–Akaike information criterion (WAIC) and leave-one-out information criterion (LOOIC). Since smaller values for both WAIC and LOOIC indicate a better model fit, it can be concluded from both inforamtion criteria that the Poisson model fits the data better: Poisson WAIC = 987, LOOIC = 988, negative binomial WAIC = 991, LOOIC = 991.

Result from the Poisson regression show that there was no difference in digit serial order recall as a function of memory type (estimate = `r round(fixef(expt2_serial_order)[2, 1], 3)`, 95% CrI = `r round(fixef(expt2_serial_order)[2, 3], 3)`, `r round(fixef(expt2_serial_order)[2, 4], 3)`.   
